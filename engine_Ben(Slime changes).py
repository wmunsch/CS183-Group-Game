...
class Slime(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        new_pos = self.chase(player.rect)
        if new_pos :
...
ADDENEMY = pygame.USEREVENT + 1  ######user event for spawning enemies##############
pygame.time.set_timer(ADDENEMY, 5000) #######sets timer for spawn event#######################
enemies = pygame.sprite.Group()  ######used for spawning enemies#############################
...
while not done:
        for event in pygame.event.get():
                ...
                if (event.type == ADDENEMY):  #########main spawning clause#############
                    new_enemy = Slime()
                    enemies.add(new_enemy)
                    sprites_alive.add(new_enemy)
...
############### Player Collision with slime ##########################
        ...
        enemies.update()  ########needed now instead of Slime1.update()
